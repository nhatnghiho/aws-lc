...
function get_commit_changed_files() {
  ...
  changed_files=$(curl -H "Authorization: token ${GITHUB_PAT}" "https://api.github.com/repos/${GITHUB_REPO_OWNER}/${GITHUB_REPO_NAME}/commits/${COMMIT_HASH}" | jq -r '.files[].filename')
