...
function unbound_parse_test_results() {
  ...
  passed=($(echo "$results" | grep -o '^"*\* PASSED \*\* : .*$' | sed 's/^\*\* PASSED \*\* : //' >/dev/null))
  failed=($(echo "$results" | grep -o '^!! FAILED !! : .*$' | sed 's/^!! FAILED !! : //' >/dev/null))
  skipped=($(echo "$results" | grep -o '^.. SKIPPED.. : .*$' | sed 's/^.. SKIPPED.. : //' >/dev/null))
